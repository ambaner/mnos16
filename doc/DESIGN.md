...
| `0x0000:0x0800` – `0x0000:0x4FFF` | **System modules** (FS.SYS + MM.SYS + SHELL.SYS, packed sequentially with v2 relocation) |
| ~~`0x0000:0x2800` – `0x0000:0x2FFF`~~ | ~~MM.SYS (fixed)~~ — *now dynamically placed* |
| ~~`0x0000:0x3000` – `0x0000:0x4FFF`~~ | ~~SHELL.SYS (fixed)~~ — *now dynamically placed* |
...
2. Finds FS.SYS via MNFS directory, loads to next_base (starts at 0x0800)
3. Applies v2 relocations (patches absolute addresses), calls FS init — installs INT 0x81
4. Finds MM.SYS via MNFS directory, loads to next_base (after FS.SYS)
5. Applies v2 relocations, calls MM init — installs INT 0x82 memory manager handler
6. Finds SHELL.SYS via MNFS directory, loads to next_base (after MM.SYS)
7. Applies v2 relocations, jumps to SHELL.SYS entry (from header's entry_offset)

### 2.7 FS.SYS

The filesystem module (`src/fs/fs.asm`) is loaded by the kernel to the first
available module slot (currently 0x0800, overwriting the dead LOADER).
...
and contiguous allocation with high-water-mark tracking.

v2 header (generated by `pack_module.py`):
```
FS Header (v2):
  Offset 0:   'MNFS'         Magic identifier (4 bytes)
  Offset 4:   dw N           FS module size in sectors
  Offset 6:   dw flags       0x0001 (has relocation table)
  Offset 8:   dw reloc_count Number of relocation entries
  Offset 10:  dw entry_off   Entry point offset from load base
  Offset 12:  reloc_table[]  Relocation entries (16-bit offsets)
  ...         code/data      Module code (assembled at ORG 0)
```
...
The memory manager (`src/mm/mm.asm`) is loaded by the kernel to the next
available slot after FS.SYS (dynamically placed).  It owns the INT 0x82
memory management syscall interface.  Uses v2 header format (same as FS.SYS
above, with 'MNMM' magic).
...
The shell (`src/shell/shell.asm`) is loaded by the kernel to the next available
slot after MM.SYS (dynamically placed).  It provides the interactive
command-line interface.  Uses v2 header format (same as FS.SYS above, with
'MNEX' magic for the shell variant).
...
> directly into the system module area, it never returns control to the kernel,
> and it has unrestricted access to all INT vectors.  Its position is no longer
> fixed at a hardcoded address — it is dynamically placed and relocated at boot.
