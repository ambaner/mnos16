...
is destroyed and the new program takes over the TPA.

**Interface:**
- DS:SI = 11-byte filename (8.3 padded uppercase, e.g., `"EDIT    MNX"`)
- DS:DI = pointer to NUL-terminated argument string (0 = no args)

**Behavior:**
- On success: new program starts; its `ret` returns to shell
- On failure: returns to caller with CF=1, AX = error code (1–5)
- The caller's TPA is NOT modified on failure (all validation is pre-load)
- Arguments are copied to kernel scratch before TPA overwrite
